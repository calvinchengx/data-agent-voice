...
[`parity.md`](parity.md) is the authority on which of those rows are green.

## When something is wrong

| Symptom | Cause |
|---|---|
| `make up` exits, `AGORA_APP_ID invalid` | it is validated even for a graph with no RTC node, and the check is length ≠ 32 ([upstream 1](upstream-issues.md)) |
